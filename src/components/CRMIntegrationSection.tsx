              ...
              <div className="phone-mockup w-48 h-auto mx-auto">
                ...
                <div className="absolute top-[40px] left-0 right-0 bottom-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-center font-medium">{t('crm.whatsapp.data')}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-lg p-4 overflow-hidden">
                <div className="bg-primary text-white p-2 rounded-t-lg">
                  <div className="flex items-center">
                    ...
                    <div className="text-sm">{t('crm.dashboard')}</div>
                  </div>
                </div>
                <div className="p-4">
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-blue-50 p-3 rounded">
                      <div className="text-xs text-blue-600">{t('crm.stats.newLeads')}</div>
                      <div className="text-2xl font-bold">128</div>
                    </div>
                    <div className="bg-green-50 p-3 rounded">
                      <div className="text-xs text-green-600">{t('crm.stats.closedSales')}</div>
                      <div className="text-2xl font-bold">47</div>
                    </div>
                  </div>
                  <div className="bg-gray-100 rounded p-2 mb-3">
                    <div className="text-xs font-medium">{t('crm.stats.conversionRate')}</div>
                    <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                      <div className="bg-primary h-2.5 rounded-full" style={{ width: '65%' }}></div>
                    </div>
                    <div className="text-right text-xs mt-1">65%</div>
                  </div>
                  <div className="text-xs font-medium mb-1">{t('crm.stats.recentClients')}</div>
                  <ul className="text-xs">
                    <li className="border-b py-1 flex justify-between">
                      <span>{t('crm.clients.client1.name')}</span>
                      <span className="text-green-600">{t('crm.clients.client1.status')}</span>
                    </li>
                    <li className="border-b py-1 flex justify-between">
                      <span>{t('crm.clients.client2.name')}</span>
                      <span className="text-blue-600">{t('crm.clients.client2.status')}</span>
                    </li>
                    <li className="py-1 flex justify-between">
                      <span>{t('crm.clients.client3.name')}</span>
                      <span className="text-orange-600">{t('crm.clients.client3.status')}</span>
                    </li>
                  </ul>
                </div>
              </div>
